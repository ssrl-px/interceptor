...
class TestArgs:
    ...
    def test_default_args(self):
        ...
        assert (
            " ".join(commands)
            == "--n_proc 10 --host localhost --port 9999 --stype req --last_stage "
            "spotfinding"
        )
    ...
    def test_presets_sans_ui(self):
        ...
        assert (
            " ".join(commands)
            == "--n_proc 144 --host bl121splitter --port 8121 --stype req "
            "--last_stage indexing"
        )

    def test_presets_with_ui(self):
        argstring = "-b 12-1 -e jet -u dhs"
        commands = self.make_command_line(argstring)
        assert (
            " ".join(commands)
            == "--n_proc 190 --host bl121splitter --port 8121 --stype req --uihost "
            "localhost --uiport 9998 --uistype push --last_stage spotfinding"
        )

    def test_default_with_mpi(self):
        command = self.make_mpi_command_line("")
        assert (
            " ".join(command) == "mpirun --map-by core --bind-to core --rank-by "
            "core --np 10 connector --host localhost --port 9999 --stype req "
    ...
    def test_presets_with_mpi(self):
        ...
        assert (
            " ".join(command)
            == "mpirun --map-by core --bind-to core --rank-by core --np 190 "
            "connector --host bl121splitter --port 8121 --stype req --uihost "
            "localhost --uiport 9997 --uistype push --last_stage spotfinding"
        )

    def test_mpi_with_binding(self):
        argstring = "--mpi_bind 0, 3, 12, 24-47, 72-191 -b 12-1 -e jet -u dhs"
        command = self.make_mpi_command_line(argstring)
        assert (
            " ".join(command)
            == "mpirun --cpu-set 0,3,12,24-47,72-191 --bind-to cpu-list:ordered "
               "--rank-by core --np 147 connector --host bl121splitter --port 8121 "
               "--stype req --uihost localhost --uiport 9998 --uistype push "
               "--last_stage spotfinding"
        )
